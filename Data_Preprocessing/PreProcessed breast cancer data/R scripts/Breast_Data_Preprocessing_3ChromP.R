C1 = 0
C1 <- read.delim("B13/B13_IDC.txt")
C1 = C1[-1,]
C1 = C1[,-13]


colnames(C1) <- c("chrom1","chrom2","chrom3","gene1","gene2","gene3","gene4","gene5","gene6","gene7","gene8","cellnum")
...
C1 <- C1[do.call(order, c(data.frame(C1[,4:12]))),]

write.table(C1, file="P_B13_IDC.txt", row.names=FALSE)
